...
% count the number of frames in the miniscope and behavioral videos
    function [msNvideos,msNframes,bhNvideos,bhNframes,mean_ms,mean_bh] = read_videos(vd,N0,N1)
        ...
        aviFiles = dir([vd '\*.avi']);
        matFiles = dir([vd '\qualitycheck\*.mat']);
        ...
        % otherwise/also use the mat files.
        if length(matFiles)>0
            file_type = 'mat';
            for i1=1:length(matFiles)
                my_name = matFiles(i1).name;
                
                % separate miniscope and behavioral video files
                if contains(my_name,'ms_mean')
                    % strip the name to extract the video number
                    my_name = my_name(1:end-4);
                    my_name = my_name(8:end);
                    % and store the number
                    msNums = [msNums str2num(my_name)];
                    
                elseif contains(my_name,'bh_mean')
                    % strip the name to extract the video number
                    my_name = my_name(1:end-4);
                    my_name = my_name(8:end);
                    % and store the number
        ...
        if isempty(bhNums) || isempty(msNums)
            disp('There are no videos or mean files in this folder. Stopping here.')
            return
        end
        ...
        for i1=1:msNvideos
            ...
            mat_name = [vd '\qualitycheck\msCam' num2str(msNums(i1)) '.mat'];
            mean_file = [vd '\qualitycheck\ms_mean' num2str(msNums(i1)) '.mat'];
            
            % extract the number of frames in the video (from the video
            % itself of the mat mean file)
            if strcmp(file_type,'avi')
                msvidObj = VideoReader(avi_name);
                msNframes(i1) = msvidObj.NumberOfFrames;
            else
                ms_mean_signal = [];
                load([vd '\qualitycheck\ms_mean' num2str(i1) '.mat']);
                msNframes(i1) = length(ms_mean_signal);
            end
            ...
            elseif isfile(avi_name)
                ...
                try
                    ...
                    dims = length(size(temp));
                    if dims==3
                        % bw (row, column and time)
                        temp = mean(temp,1);
                        temp = squeeze(mean(temp,2))';
                    else
                        % color: row, column, color and time
                        temp = squeeze(mean(mean(mean(temp,1),2),3))';
                    end
        ...
        for i1=1:bhNvideos
            ...
            mat_name = [vd '\qualitycheck\behavCam' num2str(bhNums(i1)) '.mat'];
            mean_file = [vd '\qualitycheck\bh_mean' num2str(bhNums(i1)) '.mat'];
            ...
            if strcmp(file_type,'avi')
                ...
                bh_mean_signal = [];
                load([vd '\qualitycheck\bh_mean' num2str(i1) '.mat']);
                bhNframes(i1) = length(bh_mean_signal);
            end
