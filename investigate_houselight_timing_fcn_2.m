...
% 0) extract the signal from the synchronization procedure.
meansig = [];
meanfiles = dir([datadir '\qualitycheck\bh_mean*.mat']);
for f=1:length(meanfiles)
    load([meanfiles(f).folder '\bh_mean' num2str(f) '.mat'])
    meansig = [meansig bh_mean_signal];
end

% 1) normalize the signals to bring those for the two camera types (VL vs IR) closer
...
Nt = length(meansig);
houselight = zeros(1,Nt);
for t=2:Nt
    if houselight(t-1)==0 && any(t==aonsets)
        houselight(t) = 1;
    else
        if houselight(t-1)==1 && any(t==aoffsets)
            houselight(t) = 0;
        else
            houselight(t) = houselight(t-1);
        end
    end
end
